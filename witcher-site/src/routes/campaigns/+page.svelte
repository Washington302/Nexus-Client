<script lang="ts">
	...
	async function handleCreate() {
		...
		createError = null;
		try {
			const created = await api.campaign.create({ name: newName, gameSystem: 'WITCHER' });
...
<div class="page">
	<div class="list-header">
		<h1 class="list-title">Campaigns</h1>
		<p class="list-subtitle">Your contracts and their rosters</p>
	</div>

	{#if loading}
		<div class="prompt-card"><p>Loading campaigns...</p></div>
	{:else if loadError}
...
{#if showNewCampaign}
	<div
		class="modal-overlay"
		onclick={() => (showNewCampaign = false)}
		onkeydown={(e) => e.key === 'Escape' && (showNewCampaign = false)}
		role="presentation"
	>
		<div
			class="modal-box"
		...
		>
			...
			<div class="field-group">
				...
				<input
					...
					required
					class="comic-input"
					placeholder="e.g. The Path of the Silver Sword"
				/>
			</div>
