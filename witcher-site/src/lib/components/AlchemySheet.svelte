				...
				{#each draft.alchemicalItems as item, i (item.id)}
					<div class="list-card">
						...
						<button
							...
							onclick={() =>
								(draft.alchemicalItems = draft.alchemicalItems.filter((_, x) => x !== i))}>✕</button
						>
					</div>
				{/each}
				<button
					type="button"
					class="add-row-btn"
					onclick={() => draft.alchemicalItems.push(createDefaultAlchemicalItem())}
					>+ Add Dose</button
				>
			{/snippet}
		</SheetSection>
		...
		>
			...
			{#snippet edit()}
				...
				<button
					type="button"
					class="add-row-btn"
					onclick={() => draft.activeAlchemyEffects.push(createDefaultEffect())}
					>+ Add Effect</button
				>
			{/snippet}
		</SheetSection>
				...
				{#each draft.recipes as formula, i (formula.id)}
					<div class="list-card">
						...
						{#each formula.components as comp, ci (comp.id)}
							<div class="bg-row">
								...
								<button
									...
									onclick={() =>
										(formula.components = formula.components.filter((_, x) => x !== ci))}>✕</button
								>
							</div>
						{/each}
						<button
						...
						<label class="finish-creation" class:disabled={memoriseBlocked(formula)}>
							...
							<span>
								...
								{#if memoriseBlocked(formula)}
									<strong>At capacity ({memorised}/{memoryCap})</strong>; un-memorize another first.
								{/if}
							</span>
						</label>
						...
						<button
							...
							onclick={() => (draft.recipes = draft.recipes.filter((_, x) => x !== i))}>✕</button
						>
					</div>
				{/each}
				<button
					type="button"
					class="add-row-btn"
					onclick={() => draft.recipes.push(createDefaultRecipe())}>+ Add Formula</button
				>
			{/snippet}
		</SheetSection>
